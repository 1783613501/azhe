...
function uiGetshopList(data, type) {
  ...
  if (type == "重置") {
    ...
    shopListData.formData.fiveStarPraise = -1;
  ...
  if (type == "确认驳回") {
    ...
    // console.log(ids,data,orderViewDialog.visible,activeTaskList,activeTask);
    // debugger;
    ...
    Tool.axios(
      ...
      res => {
        if (res.data.code == 0) {
          ...
          orderViewDialog.data.signUpBeginDate = res.data.data.signUpBeginDate;
          orderViewDialog.data.signUpEndDate = res.data.data.signUpEndDate;
          orderViewDialog.title = type;
          orderViewDialog.visible = true;
        }
      }
    ...
    that.$refs['orderForm'].validate((valid) => {
      if (valid) {
        Tool.axios(
          ...
          "post",
          { ids: [activeTask[0].id], points: data, code: [activeTask[0].id] },
          res => {
            if (res.data.code == 0) {
              that.$message.error(res.data.msg);
            } else {
...
let uiGetshopTableData = () => {
  ...
  Tool.axios(
    `tbms/b/orders?businessCode=${businessCode}&businessName=${businessName}&mediaType=${mediaType}&userName=${userName}&phone=${phone}&orderNo=${orderNo}&fiveFavorable=${fiveStarPraise}&orderFlag=${orderStatus}&signUpTimeStart=${beginDate ? new Date(beginDate).getTime() : ""
    }&signUpTimeEnd=${endDate ? new Date(endDate).getTime() : ""
    }&favourableComment=${favourableComment}&status=${pageType}&dealCode=&pageIndex=${pageIndex}&pageSize=${pageSize}`,
    "get",
    {},
    res => {
      res.data.data.data.forEach(element => {
        ...
        element.isFiveStar = element.fiveFavorable == 1 ? "是" : "未知";
      });
...
let passOrder = item => {
  if (item.length > 1) {
    ...
    Tool.axios(
      `tbms/b/orders/examine`,
      "post",
      { ids: [item[0].id+""], points: item[0].taskRuleReturn , code: [item[0].id]},
      res => {
        if (res.data.code == 0) {
          that.$message.error(res.data.msg);
        } else {
    ...
    row: [
      {
        ...
        data: [
          ...
          },
          {
            ...
            data: {
              text: "",
              vHtml: `<i class='el-icon-warning' style='color:#1890ff'></i> 已选择 ${chooseShop.num
                } 项`,
              busEmit: CreatedBusOn,
              num: chooseShop,
              style: {
